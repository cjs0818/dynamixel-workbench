    ...
    :node_handle_("")
{
  ...
  dxl_wb_->addSyncWrite("Goal_Position"); // HERE
...
void PositionControl::jointStatePublish()
{
  ...
  for (int index = 0; index < dxl_cnt_; index++)
    present_position[index] = dxl_wb_->itemRead(dxl_id_[index], "Present_Position"); // << HERE

  int32_t present_velocity[dxl_cnt_] = {0, };

  for (int index = 0; index < dxl_cnt_; index++)
    present_velocity[index] = dxl_wb_->itemRead(dxl_id_[index], "Present_Velocity"); // << HERE
...
void PositionControl::goalJointPositionCallback(const sensor_msgs::JointState::ConstPtr &msg)
{
  ...
  dxl_wb_->syncWrite("Goal_Position", goal_dxl_position); // << HERE
}
